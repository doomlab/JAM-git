...
if ($totaltime < 1500000)###25 = 1500000
	{
	if ($answer[$i] == $check[$i])
		{
...
#######entering file names 
#~ print "Please enter the number of the file where the data is stored:\n";
#~ $fileopen = <STDIN>;
#~ chomp $fileopen;
$fileopen = $j.".dat";
...
#~ for ($i=0; $i<@trial; $i++)
#~ {
#~ print $trial[$i], " ", $answer[$i], " ",  $check[$i], "\n";
#~ }
...
#~ print "Would you like to do another one? 1 = yes, 2 = no";
#~ $enter = <STDIN>;
