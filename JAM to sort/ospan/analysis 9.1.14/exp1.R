...
exp1 <- read.csv("C:/Users/Doom-Lab/OneDrive/projects/JAM/ospan/analysis 9.1.14/exp1.csv")
...
associates2<-scale(associates, scale = F)


##issues at this point
ospan2<-scale(ospan, scale = F)
ravens2<-scale(ravens, scale = F)
strict2<-scale(strict, scale = F)


fun <-lmer(answer~associates2+ospan2+ravens2+associates2*ospan2*ravens2+(1|part_no)+(1|cue), REML=FALSE, na.action=na.omit)
summary(fun)
